...
def plot_additional_visualizations(neural_lengths, label_lengths, label_counts):
    ...
    plt.tight_layout()
    os.makedirs("visualizations", exist_ok=True)
    plt.savefig("visualizations/additional_analysis.png", dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    print(f"[Visualization] Using device: {device}")

    print("\n[Visualization] Discovering training files...")
    ...
    print("\n[Visualization] Initializing DataLoader...")
    ...
    print("\n[Visualization] Loading Model...")
    model = build_model(config, device)
    
    try:
        latest_ckpt = get_latest_checkpoint()
        print(f"[Visualization] Loading checkpoint from: {latest_ckpt}")
