...
def objective(trial: optuna.Trial, model_type: str):
    ...
    if not train_files:
        raise RuntimeError(
            f"No train files found under {trial_config.DATA_DIR}/{trial_config.SESSION_GLOB}/{trial_config.TRAIN_FILENAME}")
    ...
    with mlflow.start_run(run_name=f"trial_{trial.number}", nested=True) as run:
        ...
        try:
            ...
            for epoch in range(1, trial_config.num_epochs + 1):
                ...
                mlflow.log_metric("lr", current_lr, step=epoch, run_id=run_id)

                if val_loss < best_val_loss:
                    best_val_loss = val_loss
                    best_val_acc = val_acc
                    patience_counter = 0
                    mlflow.log_metric("best_val_loss", best_val_loss, step=epoch, run_id=run_id)
                else:
                    patience_counter += 1
            ...
            mlflow.log_metric("final_best_val_loss", best_val_loss, run_id=run_id)
