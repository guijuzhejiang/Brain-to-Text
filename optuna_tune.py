...
def objective(trial: optuna.Trial, model_type: str):
    ...
    trial_config.batch_size = trial.suggest_categorical("batch_size", [8, 16, 24])

    if model_type == "Transformer":
        # d_model choices based on comment: 512, 768
        trial_config.d_model = trial.suggest_categorical("d_model", [512, 768])
        
        # NOTE: d_model must be divisible by nhead
        if trial_config.d_model == 512:
            trial_config.nhead = trial.suggest_categorical("nhead", [8])
        else: # 768 (768 is divisible by both 8 and 12)
...
def main():
    ...
    study_name = f"brain_to_text_{model_type.lower()}_tuning"
